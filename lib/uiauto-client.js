...
import net from 'net';
import { mkdirp, rimraf } from 'appium-support';
    ...
    return new Promise((resolve, reject) => {
      let cb = (result) => {
        ...
        if (result.status === 0) {
          ...
          reject(errorFromCode(result.status, result.value));
        } else {
          reject(new Error(result.value));
        }
      };
      // TODO: cb should not be needed, this can be implemented using promise only
